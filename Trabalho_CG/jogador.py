...
class Jogador:
    def __init__(self):
        ...
        self.player_pos = glm.vec3(7.0, 0.5, 7.0)
        self.rotate = 0.0
    ...
    def update_cam(self, shader):
        ...
        cam_x = self.player_pos.x + self.camera_distance * glm.sin(self.camera_angle)# deslocamento horizontal (eixo X) da câmera
        cam_z = self.player_pos.z + self.camera_distance * glm.cos(self.camera_angle) # deslocamento horizontal (eixo Z) da câmera

        # Y representa a altura da câmera em relação ao jogador
        target_y = self.player_pos.y + 5.0  # altura desejada da câmera (acima do jogador)
        ...
        view = glm.lookAt(camera_pos,   # posição da câmera no mundo
                        self.player_pos,  # ponto que a câmera está olhando (alvo)
    ...
    def move(self, direction, delta_time, rampas=[]):
        ...
        if glm.length(move_vec) > 0:
            ...
            self.player_pos += move_vec * speed * delta_time  # desloca o jogador
    ...
    def update(self, delta_time, rampas=[]):
        ...
        self.player_pos.y += self.vel_y * delta_time
        ...
        altura_rampa = None
        for rampa in rampas:
            h = rampa.get_height_at(self.player_pos.x, self.player_pos.z)
            if h is not None:
                altura_rampa = h
                break
        ...
        if self.player_pos.y <= altura_alvo:
            self.player_pos.y = altura_alvo
            self.vel_y = 0
            self.on_ground = True
        else:
        ...
        # Colisão com paredes
        if hasattr(self, "paredes"):
            for parede in self.paredes:
                parede.resolve_collision(self)
    ...
    def render(self, shader):
            ...
            model = glm.mat4(1.0)
            model = glm.translate(model, self.player_pos)
